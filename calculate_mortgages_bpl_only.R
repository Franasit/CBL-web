...
# Load the dataset (no bandwidth limit)
...
# 2. Run the first-stage regressions using lm (adding ONLY factor(bpl) for state fixed effects)
mortgages_firststage_D <- lm(D ~ above + x + x_right + factor(bpl), data = mortgages)
mortgages_firststage_x_treat <- lm(x_treat ~ above + x + x_right + factor(bpl), data = mortgages)
...
mortgages_secondstage <- lm(home_ownership ~ D + x + x_treat + bpl, data = mortgages_IVpredicted)
